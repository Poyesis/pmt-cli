...
export default async function doCreate(F: EnvCliFlags, logger?: Logger) {
  ...
  if (!merged.project) {
    log.warn(
      `create: no "project" in ${F.config}. Run: pmt env init <project> <category>`
    );
  }
  ...
  for (const item of merged.envs) {
    ...
    if (newSecret) {
      ...
      log.warn(
        `create: ${name} missing or empty locally, skipping secret creation`
      );
    }
  }
