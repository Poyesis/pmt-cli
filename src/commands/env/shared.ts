...
export function makeLogger(quiet: boolean): Logger {
  return {
    info: (m) => {
...
export async function pullToString(secret: string, F: EnvCliFlags) {
  const url = `${F.baseUrl}${ensureLeadingSlash(
    F.readPath.replace("{secret}", encodeURIComponent(secret))
  )}`;
...
export async function pullToFile(
  secret: string,
  filePath: string,
  F: EnvCliFlags
) {
...
export function upsertEnvMapping(cfg: EnvsJson, name: string, secret: string) {
  const out: EnvsJson = {
    project: cfg.project || "",
...
export async function createSecretIfNeeded(
  name: string,
  merged: EnvsJson,
  F: EnvCliFlags,
  { info }: Logger
  ...
  info(
    `create: POST ${url} (project="${merged.project || ""}", category="${
      merged.category || ""
    }", name="${name}")`
  );

  const payload: any = { name, env: envText };
...
export async function pushOne(
  ...
  const envText = await fsp.readFile(filePath, "utf8");
  const url = `${F.baseUrl}${ensureLeadingSlash(
    F.pushPath.replace("{secret}", encodeURIComponent(secret))
  )}`;
...
export async function saveAndLogConfig(
  filePath: string,
  obj: EnvsJson,
  label: string,
  { info }: Logger
  ...
  info(
    `${label}: wrote ${filePath} with project="${obj.project || ""}" and ${
      obj.envs.length
    } entr${obj.envs.length === 1 ? "y" : "ies"}`
  );
}
