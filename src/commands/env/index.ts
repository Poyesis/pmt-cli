...
export default function mountEnv(parent: Command) {
  const env = parent.command("env").description("Environment management");

  const addCommon = (cmd: Command) =>
    cmd
  ...
  addCommon(
    env
      .command("init")
      .description(
        "Fetch remote env list, write local .env files, fill/merge envs.json"
      )
      .argument("<project>")
      .argument("<category>")
      .option("--overwrite", "Overwrite local files if they exist")
  ).action(async (project, category, options, cmd) => {
  ...
  addCommon(
    env
      .command("create")
      .description("Scan/merge envs.json, create missing secrets")
  ...
  addCommon(
    env
      .command("pull")
      .description(
        "Pull envs (multi via envs.json) or single secret when <secret> is provided"
      )
      .argument("[secret]")
      .option("-p, --path <file>", "Output file for single pull")
      .option(
        "--name <name>",
        "Mapping name stored in envs.json for single pull"
      )
  ).action(async (secret, options, cmd) => {
    const flags = defaultEnvFlags(optsToArgv(cmd, options));
  ...
  addCommon(
    env
      .command("push")
      .description("Push local .env* files for entries with secret")
  ).action(async (options, cmd) => {
    const flags = defaultEnvFlags(optsToArgv(cmd, options));
    await doPush(flags);
  });
...
function optsToArgv(_cmd: Command, options: Record<string, any>) {
