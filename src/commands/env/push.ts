...
import { Logger, makeLogger, pushOne } from "./shared.js";

export default async function doPush(F: EnvCliFlags, logger?: Logger) {
  const log = logger ?? makeLogger(F.quiet);

  const cfg = await safeLoadConfigObj(F.config);
  if (!cfg.envs?.length) {
    log.warn(
      `push: no entries in ${F.config} (run "pmt env init <project> <category>" and/or "pmt env create")`
    );
    return;
  }
