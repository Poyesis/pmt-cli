...
import { ensureLeadingSlash, fetchJson, fileExists, safeLoadConfigObj, saveConfigObj, } from "../../core/io.js";
import { makeLogger, pullToString, upsertEnvMapping, } from "./shared.js";
export default async function doInit(project, category, F, logger) {
    const { info } = logger ?? makeLogger(F.quiet);
    const listUrl = `${F.baseUrl}${ensureLeadingSlash(F.listPath
    ...
    info(`init: updated ${F.config} with project="${project}" and ${merged.envs.length} env entr${merged.envs.length === 1 ? "y" : "ies"}`);
}
